...
| Suggested NEW path | `C:\activerse\led-climb` (or `...\led-climb-git` if OLD already uses that folder) |
...
> Examples: `C:\activerse\led-climb`, `D:\led-climb`, a Desktop extract.  
...
(e.g. `C:\activerse\led-climb-git`) so you do not overwrite OLD before audit.
...
```powershell
$NEW = "C:\activerse\led-climb"          # adjust to actual NEW path
...
UI: `http://localhost:5175`. Stop: `STOP_GAME.bat`.
