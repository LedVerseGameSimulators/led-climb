# AGENT_SETUP_AND_RUN.md — LED Climb (Windows game machine)
...
|------|-------|
| Repo | `led-climb` |
| Clone URL | `https://github.com/LedVerseGameSimulators/led-climb.git` |
| Suggested path | `C:\activerse\led-climb` |
| API / Bridge / UI | `8002` / `8766` / `5175` |
...
> What is the full path to the **existing / old** LED Climb install on this PC  
> (the codebase that was running here before today’s git clone)?  
> Examples: `C:\activerse\led-climb`, `D:\led-climb`, a Desktop extract folder.  
...
Prefer a **new** folder if OLD already occupies `C:\activerse\led-climb`, e.g.
`C:\activerse\led-climb-git`, then swap after audit. Or rename OLD aside first.

```bat
git lfs install
mkdir C:\activerse 2>nul
cd C:\activerse
git clone https://github.com/LedVerseGameSimulators/led-climb.git
cd led-climb
git checkout main
git pull
git lfs pull
...
```powershell
$NEW = "C:\activerse\led-climb"          # the git clone (after git lfs pull)
$OLD = "C:\path\to\OLD_ROOT"             # set from Phase 1
$Report = "C:\activerse\climb-reconcile-report.txt"
...
UI: `http://localhost:5175` — leave minimized windows open.  
Stop: `STOP_GAME.bat`.
